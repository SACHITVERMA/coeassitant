...
#  AI route with fetching previous chat
...
def get_notes():
    """To Fetch all notes (Official And Community Both)"""
    conn = None
    try:
        conn = get_db_connection()
        if not conn: return jsonify([])
...
def delete_note():
    """Allows admin to delete inappropriate or incorrect notes"""
    conn = None
    try:
        data = request.json
        note_id = data.get('id')
...
def send_community_msg():
    """Saves student messages to the community_chats table"""
    conn = None
    try:
        data = request.json
        email = data.get('email')
...
def get_community_messages():
    """Fetches all community messages from the database"""
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
